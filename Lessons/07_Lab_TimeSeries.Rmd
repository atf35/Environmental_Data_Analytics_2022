...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>%  # define space delimiter by " "
...
Temp_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% # my() from lubridate package for month and year
...
summary(BRA_complete$Rainfall) # no NAs
summary(BRA_complete$Temperature_C) # no NAs
...
#There is a slight upward trend in rainfall over time, although there is a lot of variability
ggplot(BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth(method = lm)

#There is an upward trend in temperature over time, although there is a lot of variability
...
# There is seasonality, with an upward trend over time
...
BRA_rainfall_Components <- as.data.frame(BRA_rainfall_decomp$time.series[,1:3]) # convert time series into dataframe in order to plot it on ggplot

BRA_rainfall_Components <- BRA_rainfall_Components %>% 
  mutate(Rainfall = BRA_complete$Rainfall,
         Date = BRA_complete$Date)

BRA_temp_Components <- as.data.frame(BRA_temp_decomp$time.series[,1:3])

BRA_temp_Components <- BRA_temp_Components %>% 
  mutate(Temperature_C = BRA_complete$Temperature_C,
         Date = BRA_complete$Date)
...
ggplot(BRA_rainfall_Components) +
  geom_line(aes(y = Rainfall, x = Date), size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
#  geom_line(aes(y = seasonal, x = Date), color = "blue") +
  ylab("Rainfall")

ggplot(BRA_temp_Components) +
  geom_line(aes(y = Temperature_C, x = Date), size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  ylab("Temperature (C)")
...
#because we have seasonality in the data, we will apply the Seasonal Mann-Kendall detection test

# p-value < 0.05, so we can reject the null hypothesis and accept that there is a seasonal trend in the rainfall data.
BRA_rainfall_trend <- Kendall::SeasonalMannKendall(BRA_rainfall_ts)
BRA_rainfall_trend
summary(BRA_rainfall_trend)

# p-value < 0.05, so we can reject the null hypothesis and accept that there is a seasonal trend in the temperature data.
