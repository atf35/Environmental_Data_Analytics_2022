...
counties_sf<- st_read('./Data/Spatial/cb_2018_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties (categorical names)
...
ggplot(counties_sf, aes(color = "AWATER")) +
    geom_sf()
```
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #read FIPS as factors instead of numbers to keep the leading 0
...
#View the outputs
ggplot() +
  geom_sf(data = huc8_sf_wind, aes(fill = AvgWindEnergy))
...
#Select intersecting counties using the `filter()` command
    # "x= .," just tells R to take the counties_sf_utm dataframe and find all the counties that intersect with the y = neuse_sf 
...
triangle_HUCs <- huc8_sf_utm[triangle_co,] #select triangle_co rows, and all columns
...
#mapview(the_hucs) + mapview(the_triangle,
#                            alpha.regions = 0,
#                            color = 'red')
...
ggplot() +
  geom_sf(data = triangle_co, color = "grey") +
  geom_sf(data = triangle_HUCs.clipped, color = "red", fill = NA)
```
