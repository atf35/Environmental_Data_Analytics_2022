...
It is a common situation when someone needs to determine whether three or more populations have equal means. To answer that question, we need to conduct a formal hypothesis test.
    ...
    H_{0}: \mu_{1} = \mu_{2} = \mu_{3} = \mu_{4} = ... \mu_{k}
    H_{A}: not all \mu_{j} are equal
    
The statistical method for conducting this test is called **Analysis of Variance - ANOVA**.

Example: A sandwich shop is interested in testing to see whether the mean value for customer orders is equal at the company's four store locations. Eight orders from each store are randomly selected. 
...
* The observations are independent - that is, the occurence of any one individual value does not affect the probability that any other observation will occur
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) #p-value < 0.05, we reject the null hypothesis that the data is normally distributed
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #normally distributed
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_064"]) # normally distributed
...
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass) #not normally distributed

# Test for equal variance
# ANOVA is robust against departures from equal variance.
# bartlett.test() performs Bartlett's test of the null that the variances 
#     in each of the groups (samples) are the same.
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID) 
#p-value < 0.05, we reject the null hypothesis that the data is normally distributed
...
# group = TRUE --> group levels with the same mean
...
> Re-order the sites so that they are grouped by those with the same mean
...
summary(Litter.anova.2way2) #reject null hypothesis: mean between the groups are different
...
# use "*" to see interaction.
summary(Litter.anova.2way3) # interaction shown in summary using ":"
...
Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction) #one way anova but with ineraction
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("a", "b", "bc", "cd", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d")) +
