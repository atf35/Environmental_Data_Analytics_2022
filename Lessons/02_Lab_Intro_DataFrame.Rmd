...
(Matrices have all columns with the same data type.)
...
car_speed #list of numbers = vector (not a dataframe)
```
...
```{r}
#Option 1
class(car_speed) #numeric vector

#Option 2
is.data.frame(car_speed) #TURE/FALSE
```
...
#Option 1: use cbind() to bind columns together that have the same number of rows
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind(). For example, when there are new observations to add to the existing data. 
class(df)
df
...
```{r}
colnames(df)  #can use this function to change the names of columns too
names(df) #or simply names()
```
...
summary(df)
head(df) #show the first 6 rows of df. tail() shows the last 6 rows of the df. 

#If you know the number of the column you want you can refer to that to access column
<<<<<<< HEAD
df[,1] #call first column of the df (format is [rows, columns])
df_2 <- df[1:10, 1:2]
df_2
=======
df_2 <- df[1:10,1:2]
