...
# Filter dataset within plot building and facet by multiple variables: subset --> only use few sites without creating a new dataframe. facet_grid(y~x) --> show plots in matrix
PMplot.faceted2 <- 
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color --> contour. When we use fill for boxplots, it is difficult to see the color of squished boxplots
print(Nutrientplot3)

# Dot plot: binaxis --> which direction to stack. dodge --> pull same value points to the side without overlapping
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
