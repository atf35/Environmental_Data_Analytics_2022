...
* Mathematical representation: a time series is defined by the values $Y_1, Y_2, ...$ of a variable $Y$ at times $t_1, t_2, ...$
        Thus, $Y = F(t)$
...
* Causes of variation of Time Series Data
      ...
      + Natural calamities: earthquakes, epidemic, flood, drought, etc.
...
* **Trend**
      + General tendenc to grow or decline over a long period
      + Easy to detect
...
* **Seasonal Variation**
      + An up and down repetitive movement occuring periodically (short duration)
      + Factors that cause seasonal variation: climate and weather condition, or custom traditions and habits
* **Random Variations**
      + Erratic movements that are not predictable because they don't follow a pattern
      + Examples: strike, fire, war, flood, earthquake, etc.
...
In the context of a single variable, $Y_t$ is the original series and $Y_s$ is a lagged version of the series. *Autocovariance and autocorrelation function five information about the dependence structure of a time series.* If the covariance is significant, it means that the observation at $t_1$ is highly dependent on the observation at $t_-1$. 
...
The ACF of a stationary process $Y_t$ at lag $h$ 
        $\rho_{t,t-h} = Corr(Y_t, Y_{t-h})$
measures the linear dependency among the process variables $Y_t$ and $Y_{t-h}$.
But the dependence structure among the *intermediate variables* 
        $Y_t, Y_{t-1}, Y_{t-2},...Y_{t-h+2}, Y_{t-h+1}, Y_{t-h}$
also plays an important role on the value of the ACF.

The **partial autocorrelation function** attempts to *remove* the influence of intermediate variables. You would have only the direct correlation between $Y_t$ and $T_{t-h}$. This correlation relationship is called the *partial autocorrelation function (PACF)*. 
...
* *Long-term tendency*:
      + Increasing (upward movement)
      + Decreasing (downward movement)
...
*Slope* ($\beta_1$) and the *intercept* ($\beta_0$) are the unknown parameters, and $\epsilon_i$ is the *error term* (or residual, is the distance from point $Y_i$ to the estimated trend, $\beta_0 + \beta_1t$). 

##### Linear Trend Estimation and Removal
1. Model the trend: find $\beta_0$ and $\beta_1$. 
2. For each observation $t$ remove trend
        $Y_{detrend_t} = Y_t - (\beta_0 + \beta_1t)$
...
Assume the observed series can be represented as
        $Y_t = \mu_t + X_t$ where $E[X_t] = 0$
For moonthly seasonal data assume 12 parameters as 
        $\mu_t = \beta_1$ for $t = 1, 13, 15,...$
              $= \beta_2$ for $t = 2, 14, 16,...$
                              ...
              $= \beta_12$ for $t = 12, 24, 36,...$
The number of seasons may be less than 12. 

#### Seasonal Trend Removal

1. Model the seasonal trend
2. For each observation $t$ remove seasonal trend
        $Y_{deseason_t} = Y_t - (\sum\beta_sD_{t,s})$
        
### Series with Deterministic Trend

For both linear and polynomial trends, detrending is accomplished by running a regression and obtaining the series of residuals. The residuals will give you the detrended series. Thats what we call **trend-stationarity**. 
...
* **Mann-Kendall Test** - monotonic trend
        ...
        + Hypothesis Test: 
                $H_0: Y_t$ is $i.i.d.(stationarity)$ 
                $H_1: Y_t$ follow a trend
~more~

* **Spearman's Rank Correlation Test** - monotonic trend
      + Spearman's correlation coefficient is a statistical measure of the strenth of a monotonic relationship
      + Unlike Pearson's correlation, the relationship does not need to be linear
      + In other words, if one variable increases so does the other, it *does not matter the porportion of the increase*
      + Spearman's will always be larger than Pearson's
      + To verify a monotonic trend in your data, compute the spearman correlation between your data and the series $T$
...
* **Dickey-Fuller (ADF) Test** - unit root
      ...
      + Consider the model $Y_t = a + \phi Y_{t-1}+\epsilon_t$
      + The objective to test
              $H_0: \phi = 1$ (i.e. contain a unit root)
              $H_1: \phi < 1$ (i.e. is stationary)
