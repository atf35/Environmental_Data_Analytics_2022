...
> Answer: The output in the concole shows that all the packages that are included in a single package. For example, the tidyverse package comes with tibble, tidyr, readr, purrr, stringr, and forcats. The output also shows any conflicts that may arise with other packages. For example, the "filter()" function works with both the dplyr and stats packages, but since the tidyverse package is loaded, the "filter()" function from the dyplyr package will be used over that of the stats package. 
...
#" , stringsAsFactors = TRUE" <- add this to the end of importing the .csv to categorize data. logical: should character vectors be converted to factors?
...
> ANSWER: The README file associated with the USGS dataset on discharge at the Eno River can be found under ./Data/Metadata/. This file contains information on how the dataset was retrieved and from whom, and what data are included in the dataset. By saving it in a Metadata folder separate from the Raw data folder, one can easily find background information on the dataset without opening it or changing the original data. The README file is also useful for assigning new meaningful column names to the data that may have not been included in the raw data. For example, the raw data column names were given as a string of characters and numbers without any indication as to what they mean. The README file contains information for what these actually represent. 
...
str(USGS.flow.data) #show dataframe and all the objects in it
dim(USGS.flow.data) #how many rows and columns
length(USGS.flow.data) #how many columns

head(USGS.flow.data) #show first few rows (default 6 first rows)
head(USGS.flow.data, 10) #show first 10 rows
tail(USGS.flow.data, 5) #show last 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #show a certain block of the dataset (rows 3000-3005 and columns 3, 8, and 14) 

class(USGS.flow.data$datetime) #show datatype of single column of dataset
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #Show min/mean/max/quartiles. could point to column only with $

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: Blank cells in the spreadsheet are left blank or contain "NA" when imported into R. 
...
> Answer: It is favorable to remove NAs from a dataset when interested in research questions relating to the mean. When working with time series data, it is favorable to remove NAs from time periods that pre-date the analysis. However, generally, NAs should be retained when located in the middle of time series datasets. 

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #remove NA and store as a new dataset as to not change the raw data
dim(USGS.flow.data)
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #specify column that you want to change and the format that it is already in to convert it into the internationally recognized format. PROBLEM: R reads the dates in the 2000s instead of the 1900s when before 1969. 
```
...
```{r fixformartDate}
# convert date object back into a character object using "format()" function
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") 

# Add a "19" before objects with dates less than 191226 (--> 1919/12/26) and add a "20" before objects with dates greater than 191226 (--> 2019/12/26) *****????*****
